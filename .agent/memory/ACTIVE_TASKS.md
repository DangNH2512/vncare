...
|---|---|---|---|---|
| T-01 | Module `auth` (JWT RS256) thay `AuthenticatedGuard` stub | — | todo | Guard hiện đọc header `x-user-id`, tự chặn chạy ở production. Cũng gỡ luôn `devUserId()` ở `apps/web-client-side/app/_lib/api.ts` |
...
| T-05 | Nối comment + reaction + chat vào web client | — | todo | API đã sẵn sàng; mới chỉ post + media được nối |
